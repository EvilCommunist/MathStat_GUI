...
from python_interface.handlers_functions.standard_functions.r_functions import (mean,
...
def get_binomial_function_likelihood(data: list[int], size: float | None = None,
    ...
    try:
        if not is_int(data):
            raise Exception("Данные для Биномиального распределения должны быть целочисленными!")
    except Exception as ex:
        return ex
    if size is None:
        size = max(data)
